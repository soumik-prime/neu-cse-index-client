...
import { resetPasswordAction } from "../../../lib/action/auth.action";
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      <AuthPageHeader
        ...
        sub={`Resetting password for ${email}`}
      />
        ...
        <div className="-mt-2 mb-4 space-y-1">
          <div className="flex gap-1">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className={`h-1 flex-1 rounded-full transition-all duration-300 ${i <= strength ? STRENGTH_COLOR[strength] : "bg-gray-200"}`}
              />
            ))}
          </div>
      ...
      <AuthBtn type="submit" loading={loading} className="mt-2">
        Change password
      </AuthBtn>
    </form>
  );
