  ...
  function removeEntry(i: number) {
    confirm({
      ...
      onConfirm: () =>
        setProfile((p) => ({
          ...p,
          higherStudy: p.higherStudy.filter((_, idx) => idx !== i),
        })),
    });
  }
    ...
    <div>
      ...
      <Field label="SSC institution">
        <InstSelect
          ...
          value={profile.ssc}
          onChange={(v) => handleInstChange("ssc", v)}
        />
      </Field>
      <Field label="HSC institution">
        <InstSelect
          institutions={institutions}
          value={profile.hsc}
          onChange={(v) => handleInstChange("hsc", v)}
        />
      </Field>
        ...
        <Card key={i} className="mb-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            <Field label="Degree">
              <Input
                value={hs.degree}
                onChange={(e) => setHS(i, "degree", e.target.value)}
            ...
            <Field label="Field of study">
              <Input
                value={hs.field}
                onChange={(e) => setHS(i, "field", e.target.value)}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            <Field label="Start date">
              <Input
                type="date"
                value={hs.startDate}
                onChange={(e) => setHS(i, "startDate", e.target.value)}
              />
            </Field>
            <Field label="End date (blank if ongoing)">
              <Input
                type="date"
                value={hs.endDate}
                onChange={(e) => setHS(i, "endDate", e.target.value)}
              />
            </Field>
          </div>
        </Card>
      ...
      {showInst && (
        <InstitutionModal
          onSave={(inst) => {
            const next = [
              ...institutions,
              { ...inst, id: institutions.length + 1 },
            ];
            setInstitutions(next);
            if (pending === "ssc") set("ssc")(inst.name);
            else if (pending === "hsc") set("hsc")(inst.name);
            else if (typeof pending === "object" && pending !== null)
              setHS(pending.hs, "institution", inst.name);
            setShowInst(false);
            setPending(null);
          }}
          onClose={() => {
